          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/65 text-xs mt-0.5">Order #{order._id.slice(-8).toUpperCase()}</p>
              </div>
            </div>
            <div className="flex gap-2 mt-4">
              <span className="text-[10px] px-3 py-1 rounded-full bg-white/15 text-white font-semibold capitalize">Payment: {order.paymentStatus}</span>
              <span className="text-[10px] px-3 py-1 rounded-full bg-white/15 text-white font-semibold capitalize">Order: {order.orderStatus}</span>
            </div>
          </div>
              ...
              <TableBody>
                {orders.map((order) => (
                  <TableRow key={order._id}>
                    <TableCell className="font-mono text-xs text-gray-500">#{order._id.slice(-8).toUpperCase()}</TableCell>
                    <TableCell className="font-medium max-w-[140px] truncate">{order.productName}</TableCell>
                    <TableCell className="text-sm text-gray-600">{order.sellerInfo.name}</TableCell>
                    <TableCell className="font-semibold" style={{ color: "#3E5F47" }}>৳{order.price.toLocaleString()}</TableCell>
                    <TableCell><PaymentBadge status={order.paymentStatus} /></TableCell>
                    <TableCell><OrderBadge status={order.orderStatus} /></TableCell>
                    <TableCell className="text-center">
                     <div className="flex items-center justify-center gap-3.5">
                       <Button
                        size="sm"
                        className="rounded-full text-xs"
                        style={{ background: "linear-gradient(135deg,#3E5F47 0%,#2c4534 100%)", color: "#fff", border: "none" }}
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button size="sm" variant="destructive" className="rounded-full text-xs px-4">Cancel</Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Are you sure you want to cancel <span className="font-medium text-foreground">{order.productName}</span>? This action cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Keep Order</AlertDialogCancel>
                            <AlertDialogAction disabled={isCancelling} onClick={() => handleCancelOrder(order._id)}>
            ...
            <FadeUp key={order._id}>
              <div className="rounded-xl border bg-white shadow-sm p-4 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-mono text-xs text-gray-400 mt-0.5">#{order._id.slice(-8).toUpperCase()}</p>
                    </div>
                  </div>
                  <span className="font-bold text-sm shrink-0" style={{ color: "#3E5F47" }}>৳{order.price.toLocaleString()}</span>
                </div>
                ...
                <div className="flex items-center gap-2 pt-1">
                  <Button size="sm" className="flex-1 rounded-full text-xs" style={{ background: "linear-gradient(135deg,#3E5F47 0%,#2c4534 100%)", color: "#fff", border: "none" }} onClick={() => setSelectedOrder(order)}>View Details</Button>
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button size="sm" variant="destructive" className="rounded-full text-xs px-4">Cancel</Button>
                    </AlertDialogTrigger>
