...
import { ArrowRight,ArrowUpRight, SprayCan } from "lucide-react";
          ...
          <div className="mb-14 flex flex-col items-center justify-center text-center gap-3 ">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-[#ECEAE5] px-4 py-2  mx-auto text-center justify-center">
                <SprayCan className="h-4 w-4 text-[#3E5F47]" />
            ...
            <Link
              href="/products"
              className="hidden items-center gap-2 text-lg text-gray-800 hover:text-black transition-colors md:flex font-medium"
            >
              View all products
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {products.slice(0, 5).map((product) => (
              <StaggerItem key={product._id}>
                <Card className="overflow-hidden rounded-[32px] border-0 bg-transparent shadow-none group">
                  <CardContent className="p-2">
                    <Link href={`/products/${product._id}`}>
                      <div className="relative overflow-hidden rounded-[32px] bg-neutral-900 border border-white/5 shadow-xl transition-all duration-300 group-hover:shadow-2xl group-hover:border-white/10">
                        ...
                        <div className="absolute right-4 bottom-4 z-10 rounded-full bg-black px-3 py-1 text-xs font-medium text-white">
                          {product.category}
                        </div>
                        <Image
                    ...
                    <div className="mt-5 flex items-start justify-between gap-4 px-2">
                      <div className="flex-1">
                        <h3 className="line-clamp-1 text-lg font-semibold text-foreground transition-colors group-hover:text-emerald-950">{product.title}</h3>
